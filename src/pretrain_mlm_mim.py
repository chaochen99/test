...
from model import My_DataLoader as My_DataLoader
from model.LayoutLMv3forMLM_MIM import LayoutLMv3ForPretraining
...
def plot_graph_2(args, epoch, iter_list, mi_losses, ml_losses):
    fig = plt.figure()
    fig.patch.set_facecolor('white')
    plt.xlabel("iter")
    plt.ylabel("loss")
...
def save_loss_epcoh(args, model, epoch, iter_list, train_losses, valid_losses, \
    ml_losses, mi_losses, accesML, accesMI, optimizer, scheduler):
    ...
    plot_graph_2(args, epoch, iter_list, ml_losses, mi_losses)
...
def main(args):
    ...
    dataloaders = my_dataloader([train_data, valid_data],samplers,batch_size=[args.batch_size, args.batch_size], num_workers=[1, 1], is_trains=[True, False])
    ...
    def validation():
        ...
        with torch.no_grad():
            ...
            for batch in val_metric_logger.log_every(valid_dataloader, val_print_freq, val_header):
                ...
                text_logits, image_logits = model.forward(inputs)
            ...
            model.train()
            return ave_losses, (ave_ml, ave_mi), (ave_accML, ave_accMI)
