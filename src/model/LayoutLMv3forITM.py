...
def init_weights(obj):
    for module in obj.modules():
        ...
        if isinstance(module, (nn.Linear, nn.Conv2d)):
            ...
            module.weight.data.normal_(mean=0.0, std=obj.config.initializer_range if hasattr(obj, "config") else 0.01)
...
class LayoutLMv3(LayoutLMv3Model):
    ...
    def forward_image(self, pixel_values, bool_mi_pos=None):
        ...
        if bool_mi_pos is not None:
            ...
            # w = bool_mi_pos.reshape(batch_size, seq_len, -1).type_as(mask_tokens)
...
class LayoutLMv3ForITM(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.model = LayoutLMv3(config)
        self.HeadForITM = HeadForITM(config)
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        init_weights(self)
    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            for b in range(bs):
                ...
                input["pixel_values"] = torch.cat([input["pixel_values"], input["pixel_values"][[neg_idx]]])
                input["input_ids"] = torch.cat([input["input_ids"], input["input_ids"][[b]]])
                input["attention_mask"] = torch.cat([input["attention_mask"], input["attention_mask"][[b]]])
                input["bbox"] = torch.cat([input["bbox"], input["bbox"][[b]]])

            # select a negative text for each image
            for b in range(bs):
                input["pixel_values"] = torch.cat([input["pixel_values"], input["pixel_values"][[b]]])
                input["input_ids"] = torch.cat([input["input_ids"], input["input_ids"][[neg_idx]]])
                input["attention_mask"] = torch.cat([input["attention_mask"], input["attention_mask"][[neg_idx]]])
                input["bbox"] = torch.cat([input["bbox"], input["bbox"][[neg_idx]]])

        outputs = self.model(**input)
        outputs = outputs[0]
        logits = outputs[:, 0]
        itm_outputs = self.HeadForITM(logits)
