...
class HeadForTitle(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.transform_act_fn = nn.GELU()
        self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.decoder = nn.Linear(config.hidden_size, 24, bias = False)
...
class LayoutLMv3ForTitle(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.model = LayoutLMv3(config)
        self.HeadForTitle = HeadForTitle(config)
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        init_weights(self)
    ...
    def forward(self, input):
        ...
        title_outputs = self.HeadForTitle(text_outputs)
