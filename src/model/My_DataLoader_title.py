...
class My_Dataloader():
    ...
    def __call__(self, datasets, samplers, batch_size, is_trains, num_workers):
        loaders = []
    ...
    def _collate_fn(self, batch):
        ...
        # for i, b in enumerate(batch):
            ...
            #     print(f"maske lenght is 0!!!! and batch[i][input_ids]length is {len(batch[i]['input_ids'])}")   
        ...
        # output_dict["bool_mi_pos"] = torch.stack([b["bool_masked_pos"] for b in batch], dim=0)
        ...
        output_dict["book_type"] = torch.tensor([b["book_type"] for b in batch])
        output_dict["book_type"] = output_dict["book_type"].view(-1, 1)
        type_mask = torch.zeros(len(batch), 24).float()
        type_mask = type_mask.scatter(1, output_dict["book_type"]-1, 1.0)
        output_dict["book_type"] = type_mask
