...
from model import My_DataLoader_itm as My_DataLoader
from model.LayoutLMv3forITM import LayoutLMv3ForITM as LayoutLMv3ForPretraining
...
def plot_graph_2(args, epoch, iter_list, itm_losses):
    fig = plt.figure()
    fig.patch.set_facecolor('white')
    plt.xlabel("iter")
    plt.ylabel("loss")
    plt.plot(iter_list, itm_losses)
    plt.legend(["ITM"])
...
def save_loss_epcoh(args, model, epoch, iter_list, train_losses, valid_losses, \
    itm_losses, acces, optimizer, scheduler):
    ...
    plot_graph_2(args, epoch, iter_list, itm_losses)
...
def main(args):
    ...
    def cal_itm_loss(itm_logits, batch):
        bs = batch["input_ids"].shape[0]
        if 'itm_flag' in batch:
            itm_flag = batch['itm_flag'].to(torch.int64)
        else:
            itm_flag = torch.cat([torch.ones(bs, 1), torch.zeros(2 * bs, 1)], dim=0).to(torch.int64)

        type_mask = torch.zeros(itm_flag.shape[0], 2).float()
        labels = type_mask.scatter(1, itm_flag, 1.0)
        labels = labels.to(f'cuda:{model.device_ids[0]}')
        loss = criterion(itm_logits+ 1e-12, labels)
        acc = (itm_logits.argmax(-1) == labels.argmax(-1)).sum() / labels.shape[0]
        return loss, acc, itm_logits, labels.argmax(-1)
    ...
    def validation():
        ...
        with torch.no_grad():
            ...
            val_metric_logger.add_meter('itm_loss', train_utils.SmoothedValue(window_size=1, fmt='{value:.4f}'))
            ...
            for batch in val_metric_logger.log_every(valid_dataloader, val_print_freq, val_header):
                ...
                itm_loss, acc, itm_logits, labels = cal_itm_loss(text_logits, batch)
                val_loss = itm_loss
                losses.append(val_loss.item())
                itm_losses.append(itm_loss.item())
                acces.append(acc.item())
                val_metric_logger.update(itm_loss=itm_loss.item())
                val_metric_logger.update(val_loss=val_loss.item())
                val_metric_logger.update(acc=acc.item())
                metric_collection(itm_logits, labels)
            ave_losses = sum(losses) / len(losses)
            ave_itm = sum(itm_losses) / len(itm_losses)
            ave_acc = sum(acces) / len(acces)
            model.train()
            metrics = metric_collection.compute()
            print(metrics)
            del metric_collection
            return ave_losses, ave_itm, ave_acc, metrics['MulticlassF1Score'].item()
    ...
    if not args.model_params is None:
        ...
        itm_losses = checkpoint["itm_losses"]
    ...
    for epoch in epochs:
        ...
        metric_logger.add_meter('itm_loss', train_utils.SmoothedValue(window_size=1, fmt='{value:.4f}'))
        ...
        for i, batch in enumerate(metric_logger.log_every(train_dataloader, print_freq, header)):
            ...
            inputs = {k: batch[k].to(f"cuda:{model.device_ids[0]}") for k in ["input_ids", "bbox", "pixel_values", "attention_mask"]}
            with autocast():
                itm_logits = model(inputs)
                itm_loss, _, _, _ = cal_itm_loss(itm_logits, batch)
                loss = itm_loss
            ...
            if not skip_lr_sched:
                scheduler.step()
            optimizer.zero_grad()
            metric_logger.update(itm_loss=itm_loss.item() if isinstance(itm_loss, torch.Tensor) else itm_loss)
            metric_logger.update(train_loss=loss.item() if isinstance(loss, torch.Tensor) else loss)
            metric_logger.update(lr=optimizer.param_groups[0]["lr"])
        iter_list.append(iter)
        train_losses.append(loss.item())
        val_loss, val_itm_loss, val_acc, val_f1 = validation()
        valid_losses.append(val_loss)
        itm_losses.append(val_itm_loss)  
