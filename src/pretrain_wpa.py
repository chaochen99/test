...
def save_loss_epcoh(args, model, epoch, iter_list, train_losses, valid_losses, \
    wpa_losses, optimizer, scheduler):
...
def main(args):
    ...
    def validation():
        ...
        with torch.no_grad():
            ...
            for batch in val_metric_logger.log_every(valid_dataloader, val_print_freq, val_header):
                ...
                wpa_logits = model.forward(inputs)[0]
    ...
    for epoch in epochs:
        ...
        val_loss, indv_loss = validation()
        valid_losses.append(val_loss)
        # ml_losses.append(indv_loss[0])
        # mi_losses.append(indv_loss[1])
        wpa_losses.append(indv_loss)   
        # accesML.append(val_acc[0])
        # accesMI.append(val_acc[1])         
        print(
            f"epoch:{epoch}, iter:{iter}, {i},  train_loss: {loss.item()}, valid_loss: {val_loss}, idiv_loss:{str(indv_loss)}"
            )
