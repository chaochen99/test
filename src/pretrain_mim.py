...
from model import  My_DataLoader_mim as My_DataLoader
...
def plot_graph_2(args, epoch, iter_list, mi_losses):
    fig = plt.figure()
    fig.patch.set_facecolor('white')
    plt.xlabel("iter")
    plt.ylabel("loss")
...
def plot_graph_3(args, epoch, iter_list, accesMI):
    fig = plt.figure()
    fig.patch.set_facecolor('white')
    plt.xlabel("iter")
    plt.ylabel("acc")
...
def save_loss_epcoh(args, model, epoch, iter_list, train_losses, valid_losses, \
    mi_losses, accesMI, optimizer, scheduler):
    ...
    plot_graph_2(args, epoch, iter_list, mi_losses)
    plot_graph_3(args, epoch, iter_list, accesMI)
    save_obj = {
        "epoch": epoch,
        "iter_list": iter_list,
    ...
    def validation():
        ...
        with torch.no_grad():
            ...
            for batch in val_metric_logger.log_every(valid_dataloader, val_print_freq, val_header):
                ...
                image_logits = model.forward(inputs)[0]
            ...
            model.train()
            return ave_losses, (ave_mi), (ave_accMI)
